...
parser.add_argument("-c", "--fcascade", required=True,
                    help="path to harrcascade xml file")


# eye 경계상자 만들기 위한 모델 /////////////////////////////////////////////////////////////1.1
parser.add_argument("-e", "--ecascade", required=True,
...
# dlib의 내장 감지기를 생성
print("얼굴 감지기 로딩중..")
face_detector = cv2.CascadeClassifier(args["fcascade"])


#  눈 감지 로드 //////////////////////////////////////////////////////////////////////1.1
eye_detctor = cv2.CascadeClassifier(args["ecascade"])
...
def process(image):
    ...
    if left_fit_line != None and right_fit_line != None:
        ...
        if abs(center) > 1.6:
            center_x = int(960 / 2.0)
            center_y = int(540 / 2.0)
...
while True: # (cap.isOpened()):
    ...
    #얼굴과 눈을 검출할 그레이스케일 이미지 준비 -> haarcascade
    grayscale = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)

    # grayscale fram에서 얼굴 감지
    rects = face_detector.detectMultiScale(grayscale, scaleFactor=1.1,
                                         minNeighbors=5, minSize=(30, 30),
                                         flags=cv2.CASCADE_SCALE_IMAGE)

# 얼굴 감지 반복
    for (x, y, w, h) in rects:
        # 하르케스캐이드 얼굴 위치 표시. ////////////////////////////////////////1.1
        faceROI = grayscale[y:y+h, x:x+w]
        # faceROI로 눈 감지 적용////////////////////////////////////////////////////1.1
        eye_rects = eye_detctor.detectMultiScale(faceROI, scaleFactor=1.1,
        ...
        for (eX, eY, eW, eH) in eye_rects:
            ...
            ptA = (x + eX, y + eY)
            ptB = (x + eX + eW, y + eY + eH)
            cv2.rectangle(frame, ptA, ptB, (0, 0, 204), 2)
            
        ''' 
        # 프레임 위에 얼굴 감지 상자 그리기 ////////////////////////1.1
        ...
        cv2.putText(frame, "EAR: {:.3f}".format(EAR), (300, 320),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 204), 2)  # Fram_weight(300, 30)
...
'''
    ...
    frame = process(frame) # process defintion convert
    cv2.imshow('frame', frame)
