...
xycar_msg = Int32MultiArray() # append
yaw_limit = 5 # append
deg_limit = 5 # append

forward_move = True # append
...
while not rospy.is_shutdown():
    ...
    deg = math.degrees(math.atan(arData["DX"]/arData["DY"])) if arData["DY"] else 0
    angle = 0 
	
    if abs(deg) > 20:
       angle = yaw * 1.5 + deg * 1.5

    else:
       angle = 50 if yaw < 0 else -50

    speed = distance / 5
	
    err_deg = 0.3
    if forward_move and distance < 70:
       if distance > 40 and abs(yaw) < err_deg and abs(deg) < err_deg:
          speed = 0
          angle = 0
    else:
       forward_move = False
    if not forward_move:
       if distance > 400 or (abs(yaw) < err_deg * 0.5 and abs(deg) < err_deg * 0.5):
          forward_move = True
    else:
       speed = -30
       angle = 0
