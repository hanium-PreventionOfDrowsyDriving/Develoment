...
EAR_FRAMES = 15  # 프레임수, 값이 높을수록 감지가 늦음.
...
def get_fitline(img, f_lines):  # Finding the representative Line
    try:
        ...
        if len(lines.shape) != 1:
            ...
            x2, y2 = int(((img.shape[0] / 2 + 110) - y) /
                         vy * vx + x), int(img.shape[0] / 2 + 110)
...
def process(image):
    ...
    gray_image = cv2.cvtColor(image, cv2.COLOR_RGB2GRAY) #이진화
...
path = 'video/'
cap = cv2.VideoCapture(path + '1280_001.mp4')
        ...
        # 그렇지 않으면, 눈 종횡비가 깜박임 임계 값보다 낮지 않으므로 카운터 및 경보음을 재설정
            ...
            if EYE_DROWSINESS == False:
                cv2.putText(frame, "Detect: X", (300, 50),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 204), 2)
                            '''
        cv2.pytText(fram, "Counter{0}". format(sleep_counter), (300, 320),
                    cv.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 204), 2)
    # 프레임 표시
